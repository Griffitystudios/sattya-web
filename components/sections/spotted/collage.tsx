...
export default function KathmanduCollage({ className = "" }: KathmanduCollageProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full aspect-square max-w-3xl mx-auto ${className}`}
    >
      {/* STUPA */}
      <div ref={stupaRef} className="absolute left-1/2 -translate-x-1/2 -top-15 w-[45%] opacity-0">
        <Image src="/images/spotted/stupa.png" alt="Boudhanath Stupa" width={500} height={600} className="w-full h-auto object-contain" />
      </div>

      {/* LEFT WINDOW */}
      <div ref={leftWinRef} className="absolute left-0 top-[35%] w-[42%] z-10 opacity-0">
      ...
      {/* DOOR */}
      <div ref={doorRef} className="absolute left-1/2 -translate-x-1/2 top-[40%] w-[38%] z-30 opacity-0">
        <Image src="/images/spotted/door.png" alt="Ornate temple door" width={400} height={600} className="w-full h-auto object-contain" />
      </div>

      {/* TOP STAR */}
      <div ref={topStarRef} className="absolute right-[18%] top-[4%] w-[10%] z-40 opacity-0">
        <Image src="/images/spotted/top-star.svg" alt="" width={80} height={80} className="w-full h-auto" />
      </div>

      {/* LEFT STAR */}
      <div ref={leftStarRef} className="absolute left-[18%] top-[28%] w-[7%] opacity-0">
        <Image src="/images/spotted/left-star.svg" alt="" width={60} height={60} className="w-full h-auto" />
      </div>

      {/* RIGHT STAR */}
      <div ref={rightStarRef} className="absolute right-[15%] top-[28%] w-[12%] opacity-0">
